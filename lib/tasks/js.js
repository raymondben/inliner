  ...
  return todo.map(function scripts(script) {
    ...
    if (src) {
      ...
      if (isMinified && !inliner.options.inlinemin) {
        ...
        // ignore analytics
        return false;
      } else if (src.indexOf('mathjax') !== -1) {
        debug('skipping mathjax');
        inliner.emit('progress', 'skipping mathjax script');
        return false;
      } else if (src.indexOf('http') === 0) {
        debug('skipping external script');
        inliner.emit('progress', 'skipping external script');
        return false;
      }
